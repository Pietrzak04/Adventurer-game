	...
	if linear_velocity.y > 0:
		...
		if vertical.is_colliding() and !invertical.is_colliding():
			...
			if horizontal.is_colliding():
				...
				new_position.y = vertical.get_collision_point().y + 32
...
func _process(delta: float):
	...
	if is_on_floor():
		...
		if current_state == state.GRABING:
			animation_tree.set("parameters/State/current", 2)
		else:
			animation_tree.set("parameters/State/current", 1)
			animation_tree.set("parameters/Air/current", int(velocity.y > 0 and !double_jump))
			if double_jump:
				animation_tree.set("parameters/DoubleJump/active", true)

func _physics_process(delta: float):
	print(double_jump)
	match(current_state):
		state.MOVEMENT:
			animation_tree.set("parameters/ClimbUp/active", false)
		...
		state.GRABING:
			if Input.is_action_just_pressed("up"):
				animation_tree.set("parameters/ClimbUp/active", true)
